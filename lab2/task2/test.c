...
int main(int argc, char** argv){
    ...
    int count_characters = libCountSign(character, buffer);
    int count_clines = libCountLines(character, buffer);

    end_timer();
    print_time(result_file, count_characters, count_clines);
...
int libCountSign(char sign, char* file){
    ...
    int signCounter = 0;
    do{
        fread(&c, sizeof(char), 1, input_file1);
        if (c == sign){
            signCounter ++;
        }

    }while(feof(input_file1) == 0);
    fclose(input_file1);
...
int libCountLines(char sign, char* file){
    ...
    do{
        fread(&c, sizeof(char), 1, input_file2);
        if((int) c == '\n'){
