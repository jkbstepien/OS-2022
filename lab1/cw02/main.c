...
int main(int argc, char** argv) {
    ...
    if (argc < 1) {
    ...
    #ifdef DYNAMIC_LIBRARY
        void* handle = dlopen("../task2/mylib.so", RTLD_LAZY);
        if (!handle) {
        ...
        void (*wc_files)();
        wc_files = (void (*)())dlsym(handle,"wc_files");

        void (*load_block_to_table)();
        load_block_to_table = (void (*)())dlsym(handle,"load_block_to_table");

        void (*remove_block)();
        remove_block = (void (*)())dlsym(handle,"remove_block");

        if(dlerror() != NULL){
            fprintf(stderr, "Error while loading functions!")
            return -1;
        }
    #endif
